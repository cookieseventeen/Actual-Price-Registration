// 物件 / 社區詳情頁
import { COMMUNITY_TREND, CITY_TREND, TRANSACTIONS } from '../data/mock';
import type { Transaction } from '../data/mock';
import { fmtTotal, fmtTotalShort } from '../lib/format';
...
function CompactList({ rows, showCommunity }: { rows: Transaction[]; showCommunity?: boolean }) {
  return (
...
export function DetailView({ t, onBack, chartMode, showGrid }: DetailViewProps) {
  // 建構 12 個月趨勢（以該筆單價回推，營造合理曲線）
  const base = t.unit;
  const trend = COMMUNITY_TREND[t.community] ||
    Array.from({ length: 12 }, (_, i) => +(base * (0.86 + 0.014 * i) + (i % 3 === 0 ? -0.6 : 0.4)).toFixed(1));
  const months = CITY_TREND.months;

  // 同社區其他成交 + 周邊比價
  const sameComm = TRANSACTIONS.filter(x => x.community === t.community && x.id !== t.id);
