// 行情分析頁
import { CITY_TREND, PRICE_DISTRIBUTION, TC_DISTRICTS } from '../data/mock';
import { TrendChart, BarChart, RankBars } from './charts/Charts';
...
export function AnalysisView({ chartMode, showGrid, barRounded }: AnalysisViewProps) {
  const trend = CITY_TREND;
  const dist = PRICE_DISTRIBUTION;
  const districts = [...TC_DISTRICTS].sort((a, b) => b.avg - a.avg);
  ...
  const overview: [string, string, string, number][] = [
    ['全市均價', '34.8', '萬/坪', 3.4],
    ['最高 西屯', '42.8', '萬/坪', 3.2],
    ['年成交量', '46,800', '筆', 8.1],
    ['預售占比', '31.5', '%', 5.2],
