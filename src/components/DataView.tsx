// 資料來源 / 爬蟲任務狀態頁 + 匯出 Modal
import { useState } from 'react';
import { CRAWL_TASKS } from '../data/mock';
import type { CrawlStatus } from '../data/mock';

const STATUS_META: Record<CrawlStatus, { label: string; color: string; bg: string; icon: string }> = {
...
function StatusTag({ status }: { status: CrawlStatus }) {
...
// ── 匯出 Modal ──
export function ExportModal({ onClose, count }: { onClose: () => void; count: number }) {
  const ALL_FIELDS = ['社區/地址', '行政區', '總價', '單價', '坪數', '格局', '樓層', '屋齡', '建物型態', '車位', '成交日期', '交易類型', '資料來源'];
  const [fields, setFields] = useState<Set<string>>(() => new Set(['社區/地址', '行政區', '總價', '單價', '坪數', '格局', '成交日期']));
  const [fmt, setFmt] = useState('csv');
  const [stage, setStage] = useState<'config' | 'running' | 'done'>('config');
  ...
  const FMT: [string, string, string][] = [['csv', 'CSV', 'pi-file'], ['xlsx', 'Excel', 'pi-file-excel'], ['json', 'JSON', 'pi-code']];
