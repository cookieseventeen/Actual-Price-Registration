// Tweaks shell + form-control helpers (ported from the OreNote design bundle).
// Standalone adaptation: values persist to localStorage and the panel is opened
// via a floating launcher button (the original spoke a design-host postMessage
// protocol that doesn't exist outside Claude Design).
import { useState, useRef, useEffect, useCallback } from 'react';
import type { ReactNode } from 'react';

const STORAGE_KEY = 'shijiatong:tweaks';
...
export function useTweaks<T extends object>(defaults: T): [T, (keyOrEdits: keyof T | Partial<T>, val?: unknown) => void] {
...
// ── TweaksPanel ─────────────────────────────────────────────────────────────
// Floating, draggable shell with a launcher button.
export function TweaksPanel({ title = 'Tweaks', children }: { title?: string; children: ReactNode }) {
  ...
  if (!open) {
    return (
      <button className="twk-launch" title="調整外觀 Tweaks" aria-label="Open tweaks"
...
// TweakColor — curated color picker (single hex options).
