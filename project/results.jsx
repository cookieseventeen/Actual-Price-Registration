...
function ResultRowCard({ t, onOpen }) {
  return (
    <div className="card card-hover" style={{ padding: 'var(--pad-4)' }} onClick={() => onOpen(t)}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 4, marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--ore-surface-100)' }}>
        {[['坪數', t.ping], ['格局', t.layout], ['樓層', t.floor], ['屋齡', t.age + '年']].map(([k, v]) => (
          <div key={k}>
...
function ResultsView({ district, query, filters, toggleFilter, viewMode, sort, setSort, onOpen, setDistrict }) {
  ...
  const activeChips = [
    ...(district ? [{ group: '_district', label: district }] : []),
    ...filters.type.map(v => ({ group: 'type', label: v })),
    ...filters.layout.map(v => ({ group: 'layout', label: v })),
    ...filters.trade.map(v => ({ group: 'trade', label: v })),
  ];
...
function ViewToggle({ viewMode }) {
  const set = m => window.__setViewMode && window.__setViewMode(m);
  return (
    <div style={{ display: 'flex', border: '1px solid var(--ore-border)', borderRadius: 8, overflow: 'hidden', background: 'var(--ore-card-bg)' }}>
      {[['card', 'pi-th-large', '卡片'], ['table', 'pi-table', '表格']].map(([m, ic, lb]) => (
